...
local YSSHFolder = "YSSH"
local ConfigurationFolder = YSSHFolder.."/Configurations"
...
local YSSHLibrary = {
	...
	Theme = {
		Default = {
			TextFont = "Default", -- Default will use the various font faces used across YSSH
		...
		Light = {
			TextFont = "Gotham", -- Default will use the various font faces used across YSSH
...
local YSSH = game:GetObjects("rbxassetid://10804731440")[1]
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == YSSH.Name and Interface ~= YSSH then
			Interface.Enabled = false
			Interface.Name = "YSSH-Old"
		end
	end
else
...
local Main = YSSH.Main
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList

YSSH.DisplayOrder = 100
...
local Notifications = YSSH.Notifications

local SelectedTheme = YSSHLibrary.Theme.Default

function ChangeTheme(ThemeName)
	SelectedTheme = YSSHLibrary.Theme[ThemeName]
	for _, obj in ipairs(YSSH:GetDescendants()) do
	...
	YSSH.Main.BackgroundColor3 = SelectedTheme.Background
	YSSH.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	YSSH.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	YSSH.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	YSSH.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	YSSH.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	YSSH.Main.Topbar.Theme.ImageColor3 = SelectedTheme.TextColor
...
local function LoadConfiguration(Configuration)
	...
	for FlagName, FlagValue in next, Data do
		if YSSHLibrary.Flags[FlagName] then
			spawn(function() 
				if YSSHLibrary.Flags[FlagName].Type == "ColorPicker" then
					YSSHLibrary.Flags[FlagName]:Set(UnpackColor(FlagValue))
				else
					if YSSHLibrary.Flags[FlagName].CurrentValue or YSSHLibrary.Flags[FlagName].CurrentKeybind or YSSHLibrary.Flags[FlagName].CurrentOption or YSSHLibrary.Flags[FlagName].Color ~= FlagValue then YSSHLibrary.Flags[FlagName]:Set(FlagValue) end
				end    
			end)
		else
			YSSHLibrary:Notify({Title = "Flag Error", Content = "YSSH was unable to find '"..FlagName.. "' in the current script"})
		end
	end
end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i,v in pairs(YSSHLibrary.Flags) do
...
function YSSHLibrary:Notify(NotificationSettings)
	spawn(function()
		...
		if NotificationSettings.Actions then
			for _, Action in pairs(NotificationSettings.Actions) do
				...
				if SelectedTheme ~= YSSHLibrary.Theme.Default then
				...
				NewAction.MouseButton1Click:Connect(function()
					...
					if not Success then
						print("YSSH | Action: "..Action.Name.." Callback Error " ..tostring(Response))
					end
...
function Hide()
	Debounce = true
	YSSHLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping K", Duration = 7})
...
function YSSHLibrary:CreateWindow(Settings)
	...
	pcall(function()
		...
		if not isfolder(YSSHFolder.."/Configuration Folders") then
	...
	if Settings.Discord then
		if not isfolder(YSSHFolder.."/Discord Invites") then
			makefolder(YSSHFolder.."/Discord Invites")
		end
		if not isfile(YSSHFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(YSSHFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"YSSH RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		else
	...
	if Settings.KeySystem then
		...
		if not isfolder(YSSHFolder.."/Key System") then
			makefolder(YSSHFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			...
			if not Success then
				print("YSSH | "..Key.." Error " ..tostring(Response))
			end
		end
	end
		...
		if isfile(YSSHFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(YSSHFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
		...
		if not Passthrough then
			...
			if KeyFound then 
				...
				if Settings.KeySettings.SaveKey then
					if writefile then
						writefile(YSSHFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
					end
					YSSHLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully"})
				end
		else
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("YSSH | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					YSSHLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("YSSH | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Input.Title.Text = InputSettings.Name
		...
		--})
				...
				DropdownOption.Interact.MouseButton1Click:Connect(function()
					...
					if not Success then
						...
						print("YSSH | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Dropdown.Title.Text = DropdownSettings.Name
						TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
		...
		function DropdownSettings:Set(NewOption)
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					YSSHLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
...
CheckingForKey = true
			...
			UserInputService.InputBegan:Connect(function(input, gameProcessed)
				if not gameProcessed and CheckingForKey then
					...
					if not Success then
						...
						print("YSSH | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Keybind.Title.Text = KeybindSettings.Name
						TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			spawn(function()
				while wait() do
					if UserInputService:IsKeyDown(Enum.KeyCode[KeybindSettings.CurrentKeybind]) and not CheckingForKey then
						local Success, Response = pcall(function()
						...
						wait(KeybindSettings.Time or 0.2)
					end
				end
			end)
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					YSSHLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
			...
			Slider.Slider.BackgroundColor3 = SelectedTheme.SliderBackground
			Slider.Slider.Progress.BackgroundColor3 = SelectedTheme.SliderProgress
			Slider.Slider.UIStroke.Color = SelectedTheme.SliderStroke
			...
			local function SetValue(Value)
				local Percentage = (Value - SliderSettings.Min) / (SliderSettings.Max - SliderSettings.Min)
				Slider.Slider.Progress.Size = UDim2.new(Percentage, 0, 1, 0)
				Slider.Slider.Indicator.Text = tostring(Value)
				SliderSettings.CurrentValue = Value
			end

			SetValue(SliderSettings.CurrentValue)

			local function OnInput(input)
				local Percentage = math.clamp((input.Position.X - Slider.Slider.AbsolutePosition.X) / Slider.Slider.AbsoluteSize.X, 0, 1)
				...
				if not Success then
					...
					print("YSSH | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Slider.Title.Text = SliderSettings.Name
			...
			Slider.Slider.InputBegan:Connect(OnInput)

			local con
			Slider.Slider.InputBegan:Connect(function()
				con = UserInputService.InputChanged:Connect(OnInput)
			end)

			Slider.Slider.InputEnded:Connect(function()
				con:Disconnect()
			end)
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					YSSHLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end

			function SliderSettings:Set(value)
				local OldValue = SliderSettings.CurrentValue
				local Percentage = (value - SliderSettings.Min) / (SliderSettings.Max - SliderSettings.Min)
				Slider.Slider.Progress.Size = UDim2.new(Percentage, 0, 1, 0)
				Slider.Slider.Indicator.Text = tostring(value)
				SliderSettings.CurrentValue = value
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			Toggle.Toggle.BackgroundColor3 = SelectedTheme.ToggleBackground
			Toggle.Toggle.UIStroke.Color = SelectedTheme.ToggleDisabledOuterStroke
			...
			local function SetState(State)
				...
				if State then
					TweenService:Create(Toggle.Toggle.Indicator, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ToggleEnabled}):Play()
					TweenService:Create(Toggle.Toggle.Indicator.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {Color = SelectedTheme.ToggleEnabledStroke}):Play()
					TweenService:Create(Toggle.Toggle.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {Color = SelectedTheme.ToggleEnabledOuterStroke}):Play()
				else
					TweenService:Create(Toggle.Toggle.Indicator, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ToggleDisabled}):Play()
					TweenService:Create(Toggle.Toggle.Indicator.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {Color = SelectedTheme.ToggleDisabledStroke}):Play()
					TweenService:Create(Toggle.Toggle.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {Color = SelectedTheme.ToggleDisabledOuterStroke}):Play()
				end
			end

			SetState(ToggleSettings.CurrentValue)

			Toggle.Interact.MouseButton1Click:Connect(function()
				SetState(not ToggleSettings.CurrentValue)
				local Success, Response = pcall(function()
				...
				if not Success then
					...
					print("YSSH | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
					YSSHLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
				end
			end
	...
	for _, TopbarButton in ipairs(Topbar:GetChildren()) do
		if TopbarButton.ClassName == "ImageButton" then
			TweenService:Create(TopbarButton, TweenInfo.new(0.5, Enum.EasingStyle.Quint), {ImageTransparency = 0.8}):Play()
		end
	end
	...
	UserInputService.InputBegan:Connect(function(input, gameProcessed)
		if not gameProcessed and input.KeyCode == Enum.KeyCode.K and Hidden and not Debounce then
			Hidden = false
			spawn(Unhide)
		end
	end)
	...
	if CEnabled then
		...
		if not Success then
			YSSHLibrary:Notify({Title = "Configuration Error", Content = "No configuration file was found for this script"})
		end
	end
